...
#include <articulated_control/ControlSequenceVW.h>
...
private:
  ...
  bool enable_usb_command_tx_{false};
  std::string solved_rc_control_topic_;
  std::string solved_rc_leg_topic_;
  int solved_rc_seq_len_{10};
  double solved_rc_default_leg_length_{0.18};
  ...
  ros::Publisher solved_rc_control_pub_;
  ros::Publisher solved_rc_leg_pub_;
  ...
  void publishSolvedRcCmd(ReceiveSolvedRcCmdData & data);
