    ...
    while (1) {
        ...
        if (now_oled_errror == 0) {
            ...
            if (refresh_tick > configTICK_RATE_HZ / (OLED_CONTROL_TIME * REFRESH_RATE)) {
                ...
                x_fdb = CHASSIS.fdb.body.x;
                x_ref = CHASSIS.ref.body.x;
                x_raw_error = ChassisGetBodyXRawError();
                x_ctrl_error = ChassisGetBodyXControlError();
                OLED_printf(0, 39, "XF%+.2f XR%+.2f", x_fdb, x_ref);
                OLED_printf(0, 51, "XE%+.2f XI%+.2f", x_raw_error, x_ctrl_error);
                OLED_refresh_gram();
            }
        }
