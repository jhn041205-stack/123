...
static uint8_t LastRxBufferFS[APP_RX_DATA_SIZE];
static volatile uint32_t LastRxLengthFS = 0;
static volatile uint8_t LastRxReadyFS = 0;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  if ((Buf != NULL) && (Len != NULL)) {
    ...
    if (copy_len > 0U) {
      memcpy(LastRxBufferFS, Buf, copy_len);
    }

    LastRxLengthFS = copy_len;
    LastRxReadyFS = 1U;
  }
...
int8_t USB_Receive(uint8_t* Buf, uint32_t *Len)
{
    ...
    if (LastRxReadyFS != 0U) {
        __disable_irq();
        copy_len = LastRxLengthFS;
        if (copy_len > *Len) {
            copy_len = *Len;
        }
        if (copy_len > 0U) {
            memcpy(Buf, LastRxBufferFS, copy_len);
        }
        LastRxLengthFS = 0U;
        LastRxReadyFS = 0U;
        __enable_irq();
