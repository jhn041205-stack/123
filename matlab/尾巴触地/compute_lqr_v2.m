...
% a_t_p_val = 0.0715;             % 先用旧 lT_real 作为前向距离初值
...
% delta_t_val = 0.065972;         % 尾巴质心偏置角 (rad)
...
fprintf('使用 vpasolve 数值求解 [theta_l, theta_r, T_leg_eq, T_t_eq] ...\n');
...
A_dyn_theta_t = M_inv * dg_dtheta_t;
...
% 腿长参数查找表
% v 1.1
% theta0_fun = @(x) 272.61*x.^4 - 298.01*x.^3 + 128.02*x.^2 - 27.152*x + 2.7493;
% ld_fun     = @(x) 1.3021*x.^4 - 1.935*x.^3 + 1.1914*x.^2 - 0.0438*x + 0.0841;
% Ileg_fun   = @(x) -0.3293*x.^4 + 0.3386*x.^3 + 0.0823*x.^2 + 0.0281*x + 0.0237;
% Iyaw_fun   = @(x) -19.766*x.^4 + 16.3*x.^3 - 6.1551*x.^2 + 0.9025*x + 0.2427;
    ...
    for i = 1:num_legs
        for j = 1:num_legs
            ...
            try
                ...
                % ==================== 数值求解平衡点 ====================
                % 未知量: [theta_l, theta_r, T_leg_eq_fit, T_t_eq_fit]
                x0 = [0.2; 0.2; 0.4; -1.4];
                ...
                dg_dtheta_t_fit = double(subs(diff(g_fit, theta_t), eq_subs_fit(:,1), eq_subs_fit(:,2)));
                ...
                A_dyn_theta_b_fit = M_inv_fit * dg_dtheta_b_fit;
                A_dyn_theta_t_fit = M_inv_fit * dg_dtheta_t_fit;
