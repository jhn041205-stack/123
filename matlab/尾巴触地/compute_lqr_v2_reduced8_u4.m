...
B_dyn_red = B_dyn_full(1:4, 1:4);   % 对应 [ddX_b_h; ddphi; ddtheta_l; ddtheta_r] × [T_r_to_b,T_l_to_b,T_wr_to_r,T_wl_to_l]
...
if enable_fitting
    ...
    % [l_l, l_r, K(4x8=32个)]
    K_sample_2d = zeros(sample_size_2d, 34);
    ...
    for i = 1:num_legs
        for j = 1:num_legs
            ...
            try
                ...
                % ==================== 计算 LQR ====================
                K_fit = lqr(A_fit_num, B_fit_num, lqr_Q, lqr_R);   % 4x8
                K_fit_trans = K_fit';                              % 8x4

                % ==================== 存储 K 样本 ====================
                K_sample_2d(idx, 1) = l_l_fit;
                K_sample_2d(idx, 2) = l_r_fit;
                K_sample_2d(idx, 3:34) = K_fit_trans(:)';
    ...
    % ==================== 输出 K 拟合系数 ====================
    fprintf('// ═══════════════════════════════════════════════════════════════════════\n');
    fprintf('// K矩阵拟合系数 K_Fit_Coefficients[32][6]\n');
    fprintf('// ═══════════════════════════════════════════════════════════════════════\n');
    ...
    fprintf('float K_Fit_Coefficients[32][6] = {\n');
    for n_fit = 1:32
        row = ceil(n_fit/8) - 1;
        col = mod(n_fit-1, 8);
        fprintf('    {');
